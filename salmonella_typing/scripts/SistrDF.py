...
MDUIDREG = re.compile(r'(?P<id>[0-9]{4}-[0-9]{5,6})-?(?P<itemcode>.{1,2})?') # added change to MDU ID to handle the move from 5 to 6 digits
...
class SistrDF(object):
    ...
    def apply_filters(self, filter_list):
        ...
        self._obj['FILTERS'] = self._obj[filt_list].apply(
            lambda x: sum(~x.isnull()), axis=1)
        self._obj['serovar'] = self._obj.apply(lambda x: x.serovar if x[filt_list].isnull(
        ).all() else x[filt_list].dropna()[0], axis=1)

    def call_status(self):
        '''
        '''
        PASS = self._obj['PASS']
        REVIEW = self._obj.loc[:, self._obj.columns.str.startswith(
            'REVIEW')].any(axis=1)
        FAIL = self._obj['FAIL']
        EDGE = self._obj['EDGE']
        ...
        self._obj['STATUS'] = np.select(
            conditions, choices, default='REVIEW, INCONSISTENT')
    ...
    def output_csv(self, outname="sistr_out.csv", summary=True):
        ...
        outtab.rename(
            columns={'cgmlst_subspecies': 'subspecies'}, inplace=True)
    ...
    def output_lims(self, outname="sistr_lims.xlsx"):
        ...
        summary_header = ['ID', 'ITEMCODE', 'SEQID',
                          ...
                          'o_antigen', 'h1', 'h2', 'serogroup', 'serovar_antigen', 'serovar-original',
                          'serovar', 'STATUS'
                          ]
        ...
        review_df = self._obj.query("STATUS != 'PASS'").loc[:, summary_header].sort_values(
    ...
    def _split_id(self, row):
        ...
        if m is None:
            logger.warning(
                "The ID do not appear to follow MDU ID standards. Using SEQID directly.")
            mduid = row.SEQID
            itemcode = ''
        else:
            mduid = m.group('id')
