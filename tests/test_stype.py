...
def test_batch_cmd():
    ...
    with patch.object(RunTyping, "__init__", lambda x: None):
        ...
        cmd = f"parallel -j {args.jobs} --colsep '\\t' 'tmp_dir=$(mktemp -d -t sistr-XXXXXXXXXX) && mkdir -p {{1}} && sistr -i {{2}} {{1}} -f csv -o {{1}}/sistr.csv --tmp-dir $tmp_dir -m && rm -r $tmp_dir' :::: {args.input}"
        stype_obj.logger = logging.getLogger(__name__)
...
def test_single_cmd():
    ...
    with patch.object(RunTyping, "__init__", lambda x: None):
        ...
        stype_obj.jobs = args.jobs
        stype_obj.input = args.input
        cmd = f"tmp_dir=$(mktemp -d -t sistr-XXXXXXXXXX) && mkdir -p {args.prefix} && sistr -i {args.input} {args.prefix} -f csv -o {args.prefix}/sistr.csv --tmp-dir $tmp_dir -m && rm -r $tmp_dir"
